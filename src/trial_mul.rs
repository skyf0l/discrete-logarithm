...
pub fn discrete_log_trial_mul(n: &Integer, a: &Integer, b: &Integer) -> Result<Integer, Error> {
    discrete_log_trial_mul_with_order(n, a, b, n)
}
...
pub fn discrete_log_trial_mul_with_order(
    n: &Integer,
    a: &Integer,
    b: &Integer,
    order: &Integer,
...
mod tests {
    ...
    fn trial_mul() {
        assert_eq!(
            discrete_log_trial_mul(&587.into(), &(Integer::from(2).pow(7)), &2.into()).unwrap(),
            7
        );
        assert_eq!(
            discrete_log_trial_mul(&941.into(), &(Integer::from(7).pow(18)), &7.into()).unwrap(),
            18
        );
        assert_eq!(
            discrete_log_trial_mul(&389.into(), &(Integer::from(3).pow(81)), &3.into()).unwrap(),
            81
        );
        assert_eq!(
            discrete_log_trial_mul(&191.into(), &(Integer::from(19).pow(123)), &19.into()).unwrap(),
            123
        );
    }
}
